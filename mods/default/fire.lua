	...
	on_use = function(itemstack, user, pointed_thing)
		...
		if pointed_thing.type == "node" then
			...
			elseif minetest.get_item_group(node_under, "flammable") >= 1
				...
				minetest.set_node(pointed_thing.above, {name = "default:basic_flame"})
			end
		end
...
minetest.register_craft({
	...
	recipe = {
		{"default:steelblock_white", "default:steelblock_black"}
	}
})
...
minetest.override_item("default:steelblock_black", {
	after_destruct = function(pos, oldnode)
		pos.y = pos.y + 1
		if minetest.get_node(pos).name == "default:permanent_flame" then
			minetest.remove_node(pos)
		end
	end,
	on_ignite = function(pos, igniter)
		...
		if minetest.get_node(flame_pos).name == "air" then
			minetest.set_node(flame_pos, {name = "default:permanent_flame"})
		end
	end,
})
...
if flame_sound then
	...
	function default.update_player_sound(player)
		...
		local fpos, num = minetest.find_nodes_in_area(
			areamin,
			areamax,
			{"default:basic_flame", "default:permanent_flame"}
		)
		-- Total number of flames in radius
		local flames = (num["default:basic_flame"] or 0) +
			(num["default:permanent_flame"] or 0)
		-- Stop previous sound
	...
	minetest.register_globalstep(function(dtime)
		...
		for n = 1, #players do
			default.update_player_sound(players[n])
		end
	end)
